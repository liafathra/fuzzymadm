...
st.sidebar.markdown("---")
st.sidebar.markdown("### ⚖ Bobot Kriteria (Berdasarkan Normalisasi wj)")
...
if page == "Home":
    ...
    *Perhatian:* Pastikan Anda mengunggah atau mengedit data Crisp (nilai 60, 80, 100, dll.) di halaman *Input Data* sebelum melakukan perhitungan.
    ...
    st.download_button("⬇ Download data (.csv)", edited.to_csv().encode('utf-8'),
    ...
    if df_crisp.empty:
        ...
        if len(df_crisp.columns) != len(ws):
            ...
            try:
                ...
                st.download_button("⬇ Download hasil SAW (.xlsx)", data=buf,
    ...
    if df_crisp.empty:
        ...
        if len(df_crisp.columns) != len(ws):
            ...
            try:
                ...
                st.download_button("⬇ Download hasil WP (.xlsx)",
                                   data=buf,
